...
//Return token Macro from input string.
int detectPieceFromInput(char* str)
{
	int playerPiece = -1; //Initialize to invalid

	char lowerstr[20]; //Create empty string to place lowercase input into
	for(int i=0;i<20;i++)
	{
		lowerstr[i] = '\0'; //Initialize to null characters
	}

	for(int i=0; i<strlen(str); i++)
	{
		lowerstr[i] = tolower(str[i]); //Make input string lowercase
	}

	//Compare string, find out which one it is.
	if(strcmp("dog", lowerstr) == 0)
		playerPiece = PDOG; //Dog
	else if(strcmp("car", lowerstr)==0)
		playerPiece = PCAR; //Car
	else if(strcmp("battleship", lowerstr)==0)
		playerPiece = PBAT; //Battleship
	else if(strcmp("hat", lowerstr)==0)
		playerPiece = PHAT; //Hat
	else if(strcmp("thimble", lowerstr)==0)
		playerPiece = PTHI; //Thimble
	else if(strcmp("shoe", lowerstr)==0)
		playerPiece = PSHO; //Shoe
	else if(strcmp("iron", lowerstr)==0)
		playerPiece = PIRO; //Iron
	else if(strcmp("wheelbarrow", lowerstr)==0)
		playerPiece = PWHE; //Wheelbarrow
	else //If none of these, then ask again.
	{
		char input[20];
		for(int i=0;i<20;i++)
		{
	...
	return playerPiece; //Return macro for which piece the player has chosen.
}

//Return string from token macro.
...
//Return string from group macro.
const char* getGroup(int propertyGroupInteger)
{
	//This takes in the internal constant for a group color, and
...
//Returns 1 if roll is double, 0 else.
...
//Prints characteristics of the property that the given player is on.
void printCurrentPos(int whatPlayer)
{
	//Print where the character is
	printf("\n%s landed on %s.", players[whatPlayer].name, properties[players[whatPlayer].position].name);

	if(properties[players[whatPlayer].position].group != GSPECIAL) //If player didn't land on special property
	{
		printf("\nOwner: %s", getOwner(properties[players[whatPlayer].position].owner));//Print current owner of the property
		printf("\nView more details? (Y/N)\n"); //Ask player if they want to view details
		char response1[20]; //Initialize response string
		scanf("%9s", response1); //Ask for response
		if(isYes(response1)) //If player says yes
		{
			//List characteristics of that property
...
//Takes in a string, returns 1 if input is "y" or "yes" (toLowerCased)
int isYes(char input[20])
{
	int result = -1; //Initialize result to out of bounds
...
//Returns string containing the name of a certain player, using Player Macros.
...
//Returns 1 if the input position is chance, 0 otherwise.
int isChance(int position)
{
	int isChance = -1;
...
//Does all actions that need to take place when the player lands on a special property, such as
//GO, Chance, Jail, Income Tax, Luxury Tax, Go To Jail, and Free Parking.
...
void doChance(int playerNumber, int roll)
{
	...
	case 5:
		...
		printf("\n Make general repairs on all your properties. Total cost: %d. Your new balance is %d.\n", formula, players[playerNumber].cash);
		break;
	case 6:
...
//Performs relevant actions for Income Tax.
...
{
	printf("\nYou may have landed on Go To Jail, but you have been pardoned! OK, very cool!");
}
